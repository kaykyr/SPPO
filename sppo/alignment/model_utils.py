...
def is_adapter_model_new(model_name_or_path, revision=None):
    if isinstance(model_name_or_path, AutoModelForCausalLM) or isinstance(model_name_or_path, PeftModel):
        return False
    ...
    adapter_files = [f for f in repo_files if f.startswith("adapter_")]
